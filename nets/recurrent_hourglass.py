...
class exkp(nn.Module):
  def __init__(self, n, nstack, dims, modules, first_conv, num_classes, cnv_dim=256):
    ...
    self.pre = nn.Sequential(convolution(7, first_conv, 128, stride=2),
...
def get_hourglass(arch, first_conv, num_classes):
  if arch == 'large_hourglass':
    n=5
    nstack=2
    dims=[256, 256, 384, 384, 384, 512]
    modules=[2, 2, 2, 2, 2, 4]
  elif arch == "small_hourglass":
    n=5
    nstack=1
    dims=[256, 256, 384, 384, 384, 512]
    modules=[2, 2, 2, 2, 2, 4]
  else: raise NotImplementedError
  return exkp(n=n, nstack=nstack, dims=dims, modules=modules, 
    first_conv=first_conv, num_classes=num_classes)
